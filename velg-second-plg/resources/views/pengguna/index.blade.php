...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-600">Daftar pengguna yang terdaftar.</p>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @foreach($penggunas as $pengguna)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-sm text-gray-900">{{ $pengguna->nama }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $pengguna->email }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600">
                                {{ optional($pengguna->created_at)->format('d M Y H:i') }}
                            </td>
                            <td class="px-4 py-3 text-sm text-right">
                                @if(Route::has('penggunas.show'))
                                    <a href="{{ route('penggunas.show', $pengguna) }}"
