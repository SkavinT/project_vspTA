    ...
    <div class="max-w-3xl mx-auto px-4">
        <div class="mb-6 flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600">Perbarui data pelanggan berikut.</p>
            </div>
        ...
        <div class="rounded-lg border bg-white p-5">
                ...
                <form action="{{ route('pelanggan.update', $pelanggan) }}" method="post" class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="alamat" rows="3"
                                  class="mt-1 w-full rounded-md border-gray-300 px-3 py-2 focus:border-indigo-500 focus:ring-indigo-500">{{ old('alamat', $pelanggan->alamat) }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="email" name="email"
                                   value="{{ old('email', auth()->user()->email ?? $pelanggan->email ?? '') }}"
                                   readonly
                                   class="mt-1 w-full rounded-md border-gray-300 bg-gray-50 px-3 py-2 focus:border-indigo-500 focus:ring-indigo-500"
                                   aria-readonly="true">
                            <p class="mt-1 text-xs text-gray-500">Email mengikuti akun yang sedang login dan tidak dapat diubah.</p>
                        </div>
                    </div>
            ...
            @else
                <div class="text-sm text-red-700">
                    Route pelanggan.update belum tersedia.
                </div>
            @endif
        </div>
    </div>
