    ...
    <form action="{{ route('suppliers.update', $supplier) }}" method="post"
        ...
        <div>
            ...
            <textarea name="address" rows="3"
                      class="mt-1 w-full rounded border px-3 py-2">{{ old('address', $supplier->address) }}</textarea>
        </div>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit"
                    class="rounded-md border border-indigo-300 bg-white text-indigo-600 px-4 py-2 text-sm font-semibold hover:bg-indigo-50">
                Simpan
            </button>
        </div>
    </form>
